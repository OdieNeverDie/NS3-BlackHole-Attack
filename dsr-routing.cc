          ...
          if (addr != loopback)
            {
              ...
              m_ipv4->GetNetDevice (i)->SetPromiscReceiveCallback (MakeCallback (&DsrRouting::PromiscReceive, this));
...
//Disable DSR Retransmission mechanism 
 /*
  *
                           ...
                           Ptr<Ipv4Route> route)
{
  ...
  // When the value of 'isMalicious' is set 2,  the SourceID in the fake Route package is always the source node
  if(isMalicious==2)
  {
                                ...
                                uint8_t protocol)
{
  ...
  rreqHeader.AddNodeAddress (source);  //todo    m_mainAddress                 // Add our own address in the header
                     ...
                     Ptr<Ipv4Interface> incomingInterface)
{
  ...
  // When the malicious node finds a path to the target node, it takes it to m_routePath
  std::vector<Ipv4Address> m_routePath;
  if((isMalicious == 2))
  {
	  DsrRouteCacheEntry m_toDstCheck, kk;
	  bool findRouteCheck = m_routeCache->LookupRoute (commonTargetAddr, m_toDstCheck);
	  if(findRouteCheck)
	  {
  ...
  if (optionType == 1)        // This is the request option
    {
      ...
      // If this is a Malicious node, adding the Tag
    	  if(isMalicious == 1)
    	  {
    	  ...
    	  else if(isMalicious == 2)
    	  {
    	      ...
    	      static uint16_t m_RreqOnlyOne = 1;

    	      if((!findRoute) && (m_RreqOnlyOne == 1))
    	      {
    	    	 // When malicious node does not have a path to the Target, then reconstructs a RREQ frame
    	    	  ...
    	    	  m_rreqHeader.AddNodeAddress (maliciousIp);  //todo    m_mainAddress                 // Add our own address in the header
    	    	  m_rreqHeader.SetTarget (m_TargetAddress);
    	    	  m_requestId = m_rreqTable->CheckUniqueRreqId (m_TargetAddress);      // Check the Id cache for duplicate ones
    	    	  m_rreqHeader.SetId (m_requestId);

    	    	  m_dsrRoutingHeader.AddDsrOption (m_rreqHeader);                      // Add the rreqHeader to the dsr extension header
    	  ...
    	  else if(isMalicious == 0)
    	  {
     	  ...
     	  for(uint16_t i = 0; i < nodeList2.size(); i++)  
     	  {
     		  for(uint16_t j = 0; j <od_addressVector.size(); j++)
  ...
  else if (optionType == 96)       // This is the source route option
    {
	  ...
	  // BlackHole Type One, if data packet we received obtained the malicious tag, then we drop it all.
	  if(isMalicious==1)
	  {
	  ...
	  // BlackHole Type Two, if data packet we received obtained the malicious tag, then we drop it selectively.
	  if((isMalicious==2) && (1/* Drop Condition */))
	  {
      ...
      // Malicious Nodes tamper the legitimate route 
...
if((!tempMaskt2.IsMatch(NextHopAdd,m_mainAddress)) && flag2==1)
{
	 Ptr<Ipv4L3Protocol> tempIpv4L3;
		  ...
		  std::map<Ipv4Address, Ptr<Ipv4L3Protocol>>::const_iterator iterOdie = nodeInterface.find(NextHopAdd);
      ...
      dsrOption = GetOption (optionType);
	  optionLength = dsrOption->Process (p, packet, m_mainAddress, source, vvv, protocol, isPromisc, promiscSource);
