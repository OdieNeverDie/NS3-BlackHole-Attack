...
uint8_t DsrOptionRreq::Process (Ptr<Packet> packet, Ptr<Packet> dsrP, Ipv4Address ipv4Address, Ipv4Address source, Ipv4Header const& ipv4Header, uint8_t protocol, bool& isPromisc, Ipv4Address promiscSource)
{
  ...
  // BlackHole One, Malicious Send RREP 
  if((dsrP->GetMaliciousType()==1) || (dsrP->GetMaliciousType()==2))
  {
	  //todo Modify false shortest RREP
	  Ipv4Address maliciousIp = ipv4Address;
  ...
  if (dupRequest)
  ...
  else
    {
      ...
      if (targetAddress == ipv4Address)
        {
          ...
          isPromisc = false;
          bool a = isRouteInCache && !areThereDuplicates;  
          return rreq.GetSerializedSize ();
        }
       ...
       */
      else if (/*isRouteInCache && !areThereDuplicates*/0)  
        {
...
{
	std::cout<<"dsr->>>>> REP "<<dsrP->GetMaliciousType()<<std::endl;
